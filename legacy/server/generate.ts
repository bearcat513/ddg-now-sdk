...
import { runChoiceScript, type RunScriptOptions } from "./script";

export type Row = Record<string, unknown>;

/** Stable 32-bit hash so a text seed maps to a reproducible faker seed. */
...
/**
 * The faker instance a run draws from. `allFakers` is keyed by locale code;
 * an unknown code falls back to the default rather than failing the run, since
 * a config file may carry a locale this build of faker no longer ships.
 */
export function fakerFor(locale: string | undefined): Faker {
  const code = (locale ?? "").trim();
  if (!code) return defaultFaker;
  const pool = allFakers as unknown as Record<string, Faker | undefined>;
  return pool[code] ?? pool[code.replace("-", "_")] ?? defaultFaker;
}
...
function standardNormal(f: Faker): number {
 ...
 */
function drawNumber(f: Faker, opts: FieldOptions, defaults: NumberDefaults): number {
...
function pickWeighted(f: Faker, values: string[], weights?: number[]): string {
  ...
  // faker rejects a zero total, which a hand-typed weight list can easily hit.
...
function weightedPool(f: Faker, opts: FieldOptions, defaults: [string, number][]): string {
...
type GenContext = {
  /** The locale-bound faker every value is drawn from. */
  f: Faker;
...
/** Splits a person-shaped value into the parts faker's builders want. */
...
function generateValue(type: FieldType, opts: FieldOptions, ctx: GenContext): unknown {
  ...
  switch (type) {
    ...
    case "enum": {
      ...
      if (values.length) return pickWeighted(f, values, opts.weights);
      // Script-backed enums are filled in by `resolveChoiceScripts` before the
      // first row is built; reaching here means that step was skipped.
      if (opts.valuesFrom === "script") {
        throw new Error("Script-backed enum has no choices — resolveChoiceScripts() must run before generateRows().");
      }
      return null;
    }
    case "template":
    ...
    // servicenow
    case "nowQuery": {
      // A limit below 1 would leave faker with an empty range, so floor it at 1.
...
/**
 * Runs every enum field whose choices come from a script, and folds the result
 * back into `options.values` so `generateRows` stays synchronous.
 *
 * Identical scripts run once and share their result, which keeps a schema that
 * hits the same endpoint from several fields to a single request. Fetching is
 * deliberately *not* cached between runs: a dynamic source is only worth having
 * if each generation sees what it currently holds.
 */
export async function resolveChoiceScripts(fields: Field[], options?: RunScriptOptions): Promise<Field[]> {
  const dynamic = fields.filter(usesChoiceScript);
  if (!dynamic.length) return fields;

  // Start every distinct script before awaiting any of them, so a schema with
  // several independent sources pays for one round trip, not N in series.
  const byScript = new Map<string, ReturnType<typeof runChoiceScript>>();
  for (const field of dynamic) {
    const script = (field.options?.script ?? "").trim();
    if (!byScript.has(script)) byScript.set(script, runChoiceScript(script, options));
  }

  const resolved = new Map<string, string[]>();
  for (const field of dynamic) {
    const result = await byScript.get((field.options?.script ?? "").trim())!;
...
export function generateRows(
  config: Pick<SchemaConfig, "fields" | "rowCount"> & Partial<Pick<SchemaConfig, "seed" | "locale">>,
): Row[] {
  const f = fakerFor(config.locale);
  ...
  const count = Math.max(1, Math.min(config.rowCount || 10, 100_000));
