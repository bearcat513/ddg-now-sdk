...
import { ChevronDown, ChevronUp, GripVertical, Loader2, Play, Plus, Settings2, Trash2 } from "lucide-react";
...
import { api, type EnumPreviewResult } from "@/lib/api";
import { FUNCTION_NAMES, parseCondition, parseFormula } from "@/lib/formula";
...
const SCRIPT_STARTER = `// Runs once per generation, on the server.
// Must return an array of choices.
const response = await fetch("https://api.example.com/statuses");
if (!response.ok) {
  // Without this, an error body surfaces later as a confusing TypeError.
...
/**
 * The `values` editor for an enum: either a hand-typed list, or a snippet that
 * fetches the choices from somewhere else. The Run button hits the same code
 * path generation uses, so what it shows is what the rows will draw from.
 */
function EnumChoices({
  ...
  const [preview, setPreview] = useState<EnumPreviewResult | null>(null);
  const [running, setRunning] = useState(false);
  const fromScript = opts.valuesFrom === "script";

  async function run() {
    setRunning(true);
    setPreview(null);
    try {
      setPreview(await api.previewEnumScript(opts.script ?? ""));
    } catch (error) {
      setPreview({ ok: false, error: error instanceof Error ? error.message : "Could not run the script.", logs: [] });
    } finally {
      setRunning(false);
    }
  }
    ...
    <div className="col-span-2 flex flex-col gap-2 sm:col-span-4">
      <div className="flex items-end gap-2">
        <OptionField label="Choices from">
          <Select
            value={fromScript ? "script" : "list"}
            onValueChange={value => {
              setPreview(null);
              if (value === "script") {
                setOption("valuesFrom", "script");
                if (!opts.script?.trim()) setOption("script", SCRIPT_STARTER);
              } else {
                setOption("valuesFrom", "list");
              }
            }}
          >
            ...
            <SelectContent>
              ...
              <SelectItem value="script">Script (fetch)</SelectItem>
            </SelectContent>
          </Select>
        </OptionField>

        {fromScript && (
          <Button variant="secondary" size="sm" className="h-8" onClick={run} disabled={running}>
      ...
      {fromScript ? (
        <>
          <OptionField label="Script">
            <Textarea
              className="min-h-32 font-mono text-xs"
              spellCheck={false}
              placeholder={SCRIPT_STARTER}
              value={opts.script ?? ""}
              onChange={e => setOption("script", e.target.value)}
            />
          </OptionField>

          {preview?.ok === false && <p className="text-[11px] text-destructive">{preview.error}</p>}
          {preview?.ok && (
            <p className="text-[11px] text-muted-foreground">
              <span className="text-foreground">{preview.count}</span> choice{preview.count === 1 ? "" : "s"} in{" "}
              {preview.durationMs}ms{preview.truncated && " (truncated)"} —{" "}
              <span className="font-mono">{preview.values.slice(0, 8).join(", ")}</span>
              {preview.values.length > 8 && " …"}
            </p>
          )}
          {preview?.logs.length ? (
            <pre className="max-h-24 overflow-auto rounded border bg-background p-1.5 font-mono text-[11px] text-muted-foreground">
              {preview.logs.join("\n")}
            </pre>
          ) : null}

          <p className="text-[11px] text-muted-foreground/70">
            An async function body — use <span className="font-mono">await fetch(…)</span> and return an array of
            values. It runs on the server once per generation, so there is no CORS and{" "}
            <span className="font-mono">env.MY_TOKEN</span> reads the server&apos;s environment.
          </p>
        </>
      ) : (
