      ...
      {spans.map((span, index) => (
        <Fragment key={index}>
          {span.kind === "code" ? (
            ...
            // A document has nothing to resolve an id against, so it is shown
            // as what it is. Notes render these as live chips instead.
            <code className="rounded bg-muted px-1 py-0.5 font-mono text-[0.85em]">{span.id}</code>
          ) : span.kind === "link" ? (
            <a
              href={span.href}
