/**
 * The little Markdown an OpenAPI description carries, parsed.
 *
 * Copied from the Bun app's `lib/markdown.ts`, less the `[[record]]`
 * references its notes used — notes did not come across. Descriptions in
 * `src/server/lib/openapi.ts` are written as prose with headings, bullets and
 * `code spans`, because that is what Postman and Bruno render, and the
 * reference page renders the same document, so it reads the same marks.
 *
 * It returns blocks and spans rather than HTML: every mark reaches the page as
 * a React element, so nothing in a description can put markup into the page.
 * Anything it does not know stays as text.
 */
 ...
 *
 * Nothing here matches a single `*` or a lone backtick: a stray mark in a
 * hard-wrapped paragraph is far commoner than an intentional one, and eating
 * the rest of the line over it is the worse failure.
 */
const INLINE = /`([^`]+)`|\*\*([^*]+)\*\*|\[([^\]]+)\]\(([^)\s]+)\)/g;
 ...
 * Blocks, in order.
 *
 * Wrapped lines are joined with a space rather than kept apart: the
 * document's descriptions are hard-wrapped to fit the source file, and a line
 * break there means nothing to a reader of the rendered page — a blank line
 * does.
 */
